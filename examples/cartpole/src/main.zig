...
fn org_eval(org: *Organism) !bool {
    // Try to balance a pole now
    var fitness = run_cart(org.allocator, org.phenotype.?, true) catch return false;
    org.fitness = @as(f64, @floatFromInt(fitness));

    // Decide if it's a winner
    if (org.fitness >= @as(f64, @floatFromInt(win_balance_steps))) {
        org.is_winner = true;
    }
    ...
    if (org.is_winner) {
        ...
        var log_steps = @log(@as(f64, @floatFromInt(win_balance_steps)));
...
fn run_cart(allocator: std.mem.Allocator, net: *Network, random_start: bool) !usize {
    ...
    var steps: usize = 0;
    while (steps < win_balance_steps) : (steps += 1) {
        ...
        simulate_action(action, &x, &x_dot, &theta, &theta_dot);
...
//   - TAU seconds later.
fn simulate_action(action: u8, x: *f64, x_dot: *f64, theta: *f64, theta_dot: *f64) void {
    var x_copy = x.*;
