...
const ThreadSafeArenaAllocator = @import("threadsafe_arena_allocator.zig").ThreadSafeArenaAllocator;

const Options = std.heap.MemoryPoolOptions;
...
pub fn ThreadSafeMemoryPoolExtra(comptime Item: type, comptime pool_options: Options) type {
    return struct {
        const Pool = @This();
        ...
        pub fn init(allocator: std.mem.Allocator) Pool {
            return .{ .arena = ThreadSafeArenaAllocator.init(allocator) };
        }
        ...
        pub fn reset(pool: *Pool) void {
            ...
            const allocator = pool.arena.child_allocator.child_allocator;

            // TODO: Replace with "pool.arena.reset()" when implemented.

            pool.arena.deinit();
            pool.arena = ThreadSafeArenaAllocator.init(allocator);
...
test "memory pool: growable" {
    var pool = try ThreadSafeMemoryPoolExtra(u32, .{ .growable = false }).initPreheated(std.testing.allocator, 4);
    defer pool.deinit();
