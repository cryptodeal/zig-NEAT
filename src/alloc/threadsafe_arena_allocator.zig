...
pub const ThreadSafeArenaAllocator = struct {
    child_allocator: Allocator,
    state: State,
    mutex: std.Thread.Mutex = .{},

    /// Inner state of ThreadSafeArenaAllocator. Can be stored rather than the entire ThreadSafeArenaAllocator
    /// as a memory-saving optimization.
    pub const State = struct {
        buffer_list: std.SinglyLinkedList(usize) = .{},
        end_index: usize = 0,

        pub fn promote(self: State, child_allocator: Allocator) ThreadSafeArenaAllocator {
            return .{
    ...
    pub fn allocator(self: *ThreadSafeArenaAllocator) Allocator {
        return .{
            .ptr = self,
            .vtable = &.{
                .alloc = alloc,
    ...
    pub fn init(child_allocator: Allocator) ThreadSafeArenaAllocator {
        return (State{}).promote(child_allocator);
    }

    pub fn deinit(self: *ThreadSafeArenaAllocator) void {
        // NOTE: When changing this, make sure `reset()` is adjusted accordingly!
        var is_locked = self.mutex.tryLock();
        defer if (is_locked) self.mutex.unlock();
    ...
    pub fn queryCapacity(self: ThreadSafeArenaAllocator) usize {
        var size: usize = 0;
        var it = self.state.buffer_list.first;
    ...
    pub fn reset(self: *ThreadSafeArenaAllocator, mode: ResetMode) bool {
    ...
    fn createNode(self: *ThreadSafeArenaAllocator, prev_len: usize, minimum_size: usize) ?*BufNode {
    ...
    fn alloc(ctx: *anyopaque, n: usize, log2_ptr_align: u8, ra: usize) ?[*]u8 {
        const self: *ThreadSafeArenaAllocator = @ptrCast(@alignCast(ctx));
        _ = ra;
...
test "ThreadSafeArenaAllocator (reset with preheating)" {
    var arena_allocator = ThreadSafeArenaAllocator.init(std.testing.allocator);
...
test "ThreadSafeArenaAllocator (reset while retaining a buffer)" {
    var arena_allocator = ThreadSafeArenaAllocator.init(std.testing.allocator);
    defer arena_allocator.deinit();
